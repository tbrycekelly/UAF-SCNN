...
SparseConvNetCUDA::SparseConvNetCUDA(int dimension, int nInputFeatures, int nClasses, int pciBusID, int nTop, int nBatchProducerThreads) : deviceID(initializeGPU(pciBusID)), dimension(dimension), nInputFeatures(nInputFeatures), nClasses(nClasses), nTop(nTop), nBatchProducerThreads(nBatchProducerThreads)
{
...
void SparseConvNetCUDA::addLearntLayer(int nFeatures, ActivationFunction activationFn, float dropout, float alpha)
{
  if (activationFn != SOFTMAX)
    nFeatures = std::max(KERNELBLOCKSIZE, intRound(nFeatures, KERNELBLOCKSIZE));
  if (dropout > 0)
    dropout = 1 - (intRound(nFeatures * (1 - dropout), KERNELBLOCKSIZE) + 0.01f) * 1.0f / nFeatures;
  if (dropout < 0)
    dropout = 0;
  ...
  {
    layers.push_back(new NetworkInNetworkPReLULayer(memStream, cublasHandle, nOutputFeatures, nFeatures, dropout, alpha));
  } 
  else
  {
    layers.push_back(new NetworkInNetworkLayer(memStream, cublasHandle, nOutputFeatures, nFeatures, dropout, activationFn, alpha));
  }
  nOutputFeatures = nFeatures;
}


void SparseConvNetCUDA::addNetworkInNetworkLayer(int nFeatures, ActivationFunction activationFn, float dropout)
{
  addLearntLayer(nFeatures, activationFn, dropout, 1.0f);
}


void SparseConvNetCUDA::addConvolutionalLayer(int nFeatures, int filterSize, int filterStride, ActivationFunction activationFn, float dropout, int minActiveInputs, float poolingToFollow)
{
  if (filterSize > 1)
  {
    std::cout << layers.size() << ":";
...
void SparseConvNetCUDA::addTriangularConvolutionalLayer(int nFeatures, int filterSize, int filterStride, ActivationFunction activationFn, float dropout, int minActiveInputs, float poolingToFollow)
{
  if (filterSize > 1)
  {
    std::cout << layers.size() << ":";
...
{
  std::cout << layers.size() << ":";
  layers.push_back(new IndexLearnerLayer(memStream, cublasHandle, nOutputFeatures, nClasses));
...
float SparseConvNetCUDA::processDataset(SpatiallySparseDataset &dataset, int batchSize, float learningRate, float momentum)
{
  ...
  if (dataset.type != RESCALEBATCH)
    std::cout << dataset.name << " Mistakes:" << 100.0 * errorRate << "% NLL:" << nll << " MegaMultiplyAdds/sample:" << roundf(multiplyAddCount / dataset.pictures.size() / 1000000) << " time:" << diff / 1000000000L << "s GigaMultiplyAdds/s:" << roundf(multiplyAddCount / diff) << " rate:" << roundf(dataset.pictures.size() * 1000000000.0f / diff) << "/s" << std::endl;
  return nll;
}
void SparseConvNetCUDA::processDatasetRepeatTest(SpatiallySparseDataset &dataset, int batchSize, int nReps, std::string predictionsFilename, std::string confusionMatrixFilename)
{
  ...
  for (int rep = 1; rep <= nReps; ++rep)
  {
    ...
    auto end = std::chrono::system_clock::now();
    auto diff = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
    std::cout << dataset.name << " rep " << rep << "/" << nReps << " Mistakes: " << 100.0 * errors / dataset.pictures.size() << "% NLL " << nll / dataset.pictures.size() << " MegaMultiplyAdds/sample:" << roundf(multiplyAddCount / dataset.pictures.size() / 1000000) << " time:" << diff / 1000000000L << "s GigaMultiplyAdds/s:" << roundf(multiplyAddCount / diff) << " rate:" << roundf(dataset.pictures.size() * 1000000000.0f / diff) << "/s" << std::endl;
  }
}


void SparseConvNetCUDA::loadWeights(std::string baseName, int epoch, bool momentum, int firstNlayers)
{
...
void SparseConvNetCUDA::saveWeights(std::string baseName, int epoch, bool momentum)
{
  std::string filename = std::string(baseName) + std::string("_epoch-") + std::to_string(epoch) + std::string(".cnn");
  std::ofstream f;
...
void SparseConvNetCUDA::processIndexLearnerBatch(SpatiallySparseBatch &batch, float learningRate, float momentum, std::ofstream &f)
{
  ...
  if (batch.type == TRAINBATCH)
  {
    static_cast<IndexLearnerLayer *>(layers[n - 1])->indexLearnerIndices = batch.sampleNumbers;
...
float SparseConvNetCUDA::processIndexLearnerDataset(SpatiallySparseDataset &dataset, int batchSize, float learningRate, float momentum)
{
  ...
  auto diff = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
  if (dataset.type == TRAINBATCH)
    std::cout << dataset.name << " Mistakes:" << 100 * errorRate << "% NLL:" << nll << " MegaMultiplyAdds/sample:" << roundf(multiplyAddCount / dataset.pictures.size() / 1000000) << " time:" << diff / 1000000000L << "s GigaMultiplyAdds/s:" << roundf(multiplyAddCount / diff) << " rate:" << roundf(dataset.pictures.size() * 1000000000.0f / diff) << "/s" << std::endl;
  return nll;
}
...
void SparseConvNetCUDA::calculateInputRegularizingConstants(SpatiallySparseDataset dataset)
{
